...
function createEditionMenu () {
    ...
    if (!editionMenu){
        ...
        editionMenu.innerHTML =  '<div class="actionMenuOption">Добавить дочерний элемент</div>' +
...
function createWindow(data, type) {
    ...
    switch (type) {
        case 'main':
            ...
            newWindow.myButtonsContainer.addMyButton('Выход', function () {window.close()});
		...
		case 'combine' :
            ...
            newWindow.myButtonsContainer.addMyButton('Далее', function () {if (!checkSelection('combine')){return}; makeGeneralizedOntology(type);});
            newWindow.style.height = '300px';
            newWindow.myContent.style.height = '275px';
            newWindow.myList.style.height = '130px';
            newWindow.myList.style.cursor = 'pointer';
		...
		case 'excerpt':
			...
			newWindow.myList.fillList(data, type);
			newWindow.myButtonsContainer.addMyButton('Назад', closeMethodWindow);
            newWindow.myButtonsContainer.addMyButton('Далее', function () {if (!checkSelection('excerpt')){return}; makeGeneralizedOntology(type);});  
            newWindow.style.height = '300px';
            newWindow.myContent.style.height = '275px';
            newWindow.myList.style.height = '130px';
            newWindow.myList.style.cursor = 'pointer';
        ...
        case 'cut':
            ...
            newWindow.myButtonsContainer.addMyButton('Назад', closeMethodWindow);
            newWindow.myButtonsContainer.addMyButton('Далее', function () {if (!checkSelection('cut')){return}; makeGeneralizedOntology(type);});  //Вызов окна с результатом должен быть выполнен только после проверки корректного выполнения метода
            newWindow.style.height = '300px';
            newWindow.myContent.style.height = '275px';
            newWindow.myList.style.height = '130px';
            newWindow.myList.style.cursor = 'pointer';
        ...
        case 'find':
            ...
            searchOptionsContainer.innerHTML = '    <h5>Укажите тип поиска</h5> \
            ...
            newWindow.myButtonsContainer.addMyButton('Назад', closeMethodWindow);
            newWindow.myButtonsContainer.addMyButton('Далее', function () {if (!checkSelection('find')){return};makeGeneralizedOntology(type);});  // !!!! Добавить проверку на наличие типа поиска !!!!
...
function selectElement(elem){
    elem.className == "listElement" ? elem.className = elem.className +" selectedElement" : elem.className = "listElement";
}
...
function checkSelection(method) {
    ...
    if (method == 'find' && !(document.getElementById('dfsFlag').checked || document.getElementById('bfsFlag').checked)){
...
function excerptElements() { //Не забыть переставить везде ontology_id !!!  (или выполнить это на сервере)
	...
	var j = 0;
	for (var i = 0; i < ontology.structure.length; i++) {
		if (elementsToExcerptIDs.indexOf(ontology.structure[i].element_id) >= 0 && elementsToExcerptIDs.indexOf(ontology.structure[i].parent_element) >= 0) {
	...
	j = 0;
	for (var i = 0; i < ontology.links.length; i++) {
		if (elementsToExcerptIDs.indexOf(ontology.links[i].element_id) >= 0 && elementsToExcerptIDs.indexOf(ontology.links[i].linked_element_id) >= 0) {
	...
	if (true) {		//Запилить проверку на связность
		return true;
	}
}
//--------------------
...
function cutElements() {    //Удостовериться в корректности !!!
...
function findElement() { // !!!Не забыть про ontology_id !!!
	...
	if (document.getElementById('bfsFlag').checked) { 
		...
		for (var i = 0; i < ontology.structure.length; i++) {
			if (ontology.structure[i].level_id == level && ontology.structure[i].element_id != elementToFind.id) {
				...
				resultOntology.elements[j].element_name = document.getElementById(ontology.structure[i].element_id).innerHTML; //Опять хитрость
				j++;
			}
		}
	}
	...
	if (document.getElementById('dfsFlag').checked) {
		...
		var k = 0;
		for (var i = 0; i < ontology.structure.length; i++) {  // Добавляем элементы-потомки
			if (ontology.structure[i].parent_element == elementToFind.id) {
				resultOntology.elements[j] = {}
				resultOntology.elements[j].element_id = elementsIDs[j] = ontology.structure[i].element_id;
				resultOntology.elements[j].element_name = document.getElementById(ontology.structure[i].element_id).innerHTML; //Опять хитрость           !!! Внезапно облажался - id у DOM-элементом онтологий и элементов онтологий совпадают
				j++;
				resultOntology.structure[k] = JSON.parse(JSON.stringify(ontology.structure[i])); //Тут же добавляем иерархические связи
				k++;
			}

            if (ontology.structure[i].element_id == elementToFind.id) { //Добавляем элемент-родитель
                resultOntology.elements[j] = {}
                resultOntology.elements[j].element_id = elementsIDs[j] = ontology.structure[i].parent_element;
                resultOntology.elements[j].element_name = document.getElementById(ontology.structure[i].parent_element).innerHTML; //Опять хитрость
                j++;
                resultOntology.structure[k] = JSON.parse(JSON.stringify(ontology.structure[i])); //Тут же добавляем иерархические связи
                k++;
            }
	...
	var k = 0;
	for (var i = 0; i < ontology.links.length; i++) {
		if (elementsIDs.indexOf(ontology.links[i].element_id) >= 0 && elementsIDs.indexOf(ontology.links[i].linked_element_id) >= 0) {
